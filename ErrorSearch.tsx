...
export const ErrorSearch: React.FC<ErrorSearchProps> = ({
  ...
  const isPremium = currentUser?.subscription?.is_premium === true || currentUser?.role === 'admin' || currentUser?.is_super_admin === true || currentUser?.is_super_admin === 1;
  ...
  React.useEffect(() => {
    try {
      const keySuffix = currentUser ? `_${currentUser.id}` : '_guest';
      const savedCodes = localStorage.getItem(`free_viewed_error_codes${keySuffix}`) 
        || localStorage.getItem('free_viewed_error_codes');
      setViewedErrorCodes(savedCodes ? JSON.parse(savedCodes) : []);
      
      const savedProblems = localStorage.getItem(`free_viewed_problems${keySuffix}`) 
        || localStorage.getItem('free_viewed_problems');
      setViewedProblems(savedProblems ? JSON.parse(savedProblems) : []);
    } catch {
      setViewedErrorCodes([]);
      setViewedProblems([]);
    }
  ...
  React.useEffect(() => {
    if (isPremium) {
      ...
      if (hadPremium) {
        const keySuffix = currentUser ? `_${currentUser.id}` : '_guest';
        localStorage.removeItem(`free_viewed_error_codes${keySuffix}`);
        localStorage.removeItem(`free_viewed_problems${keySuffix}`);
  ...
  const onSelectError = (err: ErrorCode) => {
    ...
    if (!isAlreadyViewed) {
      if (viewedErrorCodes.length >= 5) {
        ...
        const keySuffix = currentUser ? `_${currentUser.id}` : '_guest';
        try {
          localStorage.setItem(`free_viewed_error_codes${keySuffix}`, JSON.stringify(updated));
  ...
  const [expandedProblemIds, setExpandedProblemIds] = React.useState<string[]>([]);

  const handleToggleProblem = (prob: CommonProblem) => {
    const isExpanded = expandedProblemIds.includes(prob.id);
    
    if (!isExpanded) {
      if (!isPremium) {
        const isAlreadyViewed = viewedProblems.includes(prob.id);
        if (!isAlreadyViewed) {
          if (viewedProblems.length >= 2) {
            ...
            const keySuffix = currentUser ? `_${currentUser.id}` : '_guest';
            try {
              localStorage.setItem(`free_viewed_problems${keySuffix}`, JSON.stringify(updated));
      ...
      setExpandedProblemIds([...expandedProblemIds, prob.id]);
    } else {
      setExpandedProblemIds(expandedProblemIds.filter(id => id !== prob.id));
    }
  };
                  ...
                  return (
                    ...
                    >
                      <div className="space-y-2.5 text-right">
                        ...
                        {!isExpanded ? (
                          <p onClick={() => handleToggleProblem(prob)} className="text-[10.5px] text-slate-500 cursor-pointer hover:underline flex items-center gap-1">
              ...
              <div className="space-y-3">
                <p className="text-xs text-rose-800 leading-relaxed font-bold bg-rose-50 border border-rose-100 p-3 rounded-xl text-right">
                  {freeLimitReachedType === 'error_code' 
                    ? '⚠️ شما از حداکثر ۵ سهمیه مشاهده رایگان کدهای خطا استفاده کرده‌اید.'
                    : '⚠️ شما از حداکثر ۲ سهمیه مشاهده رایگان عیب‌یابی مشکلات متداول استفاده کرده‌اید.'}
                </p>
                <p className="text-[11px] text-slate-500 leading-relaxed text-right">
                  کاربر گرامی، دسترسی رایگان آزمایشی (شامل ۵ کد خطا و ۲ مشکل متداول) شما به اطلاعات تخصصی سامانه به اتمام رسیده است. جهت مشاهده بی‌مرز جزییات، راهکارهای قدم‌به‌قدم برای رفع خطا، مشاهده نقشه‌های فنی ویژه و عیب‌یابی کدهای نامحدود نیاز به فعال‌سازی پکیج عضویت ویژه دارید.
                </p>
                <p className="text-[10px] text-indigo-600 bg-indigo-50/50 p-2.5 rounded-lg leading-relaxed border border-indigo-100 text-right">
                  ℹ️ تمامی دوره‌های اشتراکی ایران‌سرویس زمان‌دار هستند و بعد از طی شدن مدت معین (مانند یک ماه)، حساب شما مجدداً شامل ۵ دسترسی رایگان کدهای خطا و ۲ عیب‌یابی مشکلات متداول خواهد شد.
                </p>
              </div>
